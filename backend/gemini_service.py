...
from database import execute_query, save_conversation, get_recent_conversations
import re
...
def process_question(question, df):
    try:
        ...
        history_str = "\n".join([
            ...
            f"SQL: {conv['sql_query']}\n"
            f"Results: {conv['results']}\n"
            f"Explanation: {conv['explanation']}\n"
            f"Time: {conv['timestamp']}\n"
            for conv in history
        ])
        ...
        The data is stored in a table called 'data'.
        """
        ...
        save_conversation(question, sql_query, result_df.to_json(), explanation)
